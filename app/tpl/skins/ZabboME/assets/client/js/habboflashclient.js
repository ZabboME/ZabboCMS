...
HabbletLoader = {
    ...
    loadedHabblets: [],
    loadingStatus: [],
    needsFlashKbWorkaround: function() {
        var a = navigator.userAgent.match(/Firefox\/(\d+.\d+)/);
        var e = (a != null ? parseFloat(a[1]) : 0) >= 3.5;
        var c = navigator.userAgent.match(/Chrome/) != null;
        var d = navigator.userAgent.match(/Chrome\/(\d+.\d+.)/);
        var b = (d != null ? parseFloat(d[1]) : 0) >= 7;
        return HabbletLoader.isWindowsPlatform() && ((Prototype.Browser.Gecko && !e) || (Prototype.Browser.WebKit ? (c ? !b : true) : false))
    },
    ...
    load: function(g, c) {
        ...
        if (g == "credits") {
            e = {
                openedInNewWindow: (window == window.top)
            }
        }
        ...
        if (b) {
            var h = Builder.node("div", {
                id: "loading-" + g,
                className: "client-habblet-container loading-element"
            }, [Builder.node("img", {
                ...
                if (p > -1) {
                    ...
                    var l = function() {
                        var t = $(g) || Builder.node("div", {
                            id: g,
                            "class": "client-habblet-container contains-" + g + (i ? " draggable" : "")
                        });
                        d.appendChild(t);
                        t = $(t);
                        ...
                        setTimeout(function() {
                            ...
                            if ($("client-ui") && !$("client-ui").hasClassName("x-workaround") && i) {
                                new Draggable(t, {
                                    handle: t.select(".title")[0],
                                    starteffect: null,
                                    endeffect: null,
                                    onStart: function(y) {
                                        var w = y.element.id;
                                        y.positionBeforeDrag = HabbletLoader.loadedHabblets[w].positionedOffset()
                                    },
                                    onEnd: function(z) {
                                        var w = z.element.id;
                                        if (!HabbletLoader.isInPageBounds(w)) {
                                            var y = z.positionBeforeDrag;
                                            new Effect.Move(HabbletLoader.loadedHabblets[w], {
                                                x: y.left,
                                                y: y.top,
                                                mode: "absolute",
                                                duration: 0.5,
    ...
    isInPageBounds: function(a) {
        var b = HabbletLoader.loadedHabblets[a];
        var c = b.viewportOffset();
        if (c.left + b.getWidth() <= 20) {
            return false
        } else {
            if (c.top + 15 <= 0) {
                return false
...
var FlashHabboClient = (function() {
    ...
    var b = function() {
        ...
        swfobject.createCSS("#flash-container", "margin:0; width:100%; height:100%;")
    };
...
var FlashExternalInterface = (function() {
    ...
    var j = function(k) {
        d();
        if (typeof _gaq != "undefined") {
            _gaq.push(["_trackPageview", "/client/" + k])
        }
    };
    var i = function(n) {
    ...
    return {
        ...
        trackComufyScalar: function(k, m) {
            if (typeof comufy != "undefined") {
                var l = {};
                l[k] = m;
                comufy.storeUser(l)
            }
        },
        trackComufyMultiVal: function(k, n, l) {
            if (typeof comufy != "undefined") {
                var m = {},
                    o = {};
                o.collectionUpdateType = l;
                o.value = n;
                m[k] = o;
                comufy.storeUser(m)
            }
        ...
        onWebPageLoaded: function(k) {
            $("flash-wrapper").addClassName("hidden");
            $("page-content").addClassName("visible")
        },
        openWebPageAndMinimizeClient: function(l) {
            var k = $("page-content");
            if (k.attachEvent) {
                k.attachEvent("onload", this.onWebPageLoaded)
            } else {
                k.onload = this.onWebPageLoaded
            }
            k.src = habboReqPath + l
        },
        closeWebPageAndRestoreClient: function() {
            $("flash-wrapper").removeClassName("hidden");
            var k = $("page-content");
            k.removeClassName("visible");
            if (k.attachEvent) {
                k.detachEvent("onload", this.onWebPageLoaded)
            } else {
                k.onload = null
            }
            k.src = "about:blank";
        ...
        logEventLog: function(k) {
            new Ajax.Request(habboReqPath + "/eventlog", {
                method: "post",
                parameters: {
                    errorData: k
                }
        ...
        closeHabblet: function(k) {
            HabbletLoader.hide($(k))
        },
        ...
        authenticateFacebook: function() {
            FacebookIntegration.getAccessToken("${restFbApiHelper.facebookCookieName}", '${restFbApiHelper.extendedPermissions!""}')
        },
        listPlugins: function() {
            ...
            if (navigator.plugins != null && navigator.plugins.length > 0) {
                ...
                if (Prototype.Browser.IE) {
                    var o = null;
                    try {
                        o = new ActiveXObject("AcroPDF.PDF")
                    } catch (n) {}
                    if (!o) {
                        try {
                            o = new ActiveXObject("PDF.PdfCtrl")
                        } catch (n) {}
                    }
                    if (o) {
                        version = o.GetVersions().split(",");
                    ...
                    if (o) {
                        version = o.GetVariable("$version").substring(4);
                    ...
                    o = null;
                    try {
                        o = new ActiveXObject("QuickTime.QuickTime")
                    } catch (n) {}
                    if (o) {
                        k += "QuickTime(?),"
                    }
                    o = null;
                    var m = ["rmocx.RealPlayer G2 Control", "rmocx.RealPlayer G2 Control.1", "RealPlayer.RealPlayer(tm) ActiveX Control (32-bit)", "RealVideo.RealVideo(tm) ActiveX Control (32-bit)", "RealPlayer"];
                    for (var l = 0; l < m.length; l++) {
                        try {
                            o = new ActiveXObject(m[l])
                        } catch (n) {
                            continue
                        }
                        if (o) {
                    ...
                    if (o) {
                        ...
                        k += "RealPlayer(" + version + "),"
                    }
                    o = null;
                    try {
                        o = new ActiveXObject("SWCtl.SWCtl")
                    } catch (n) {}
                    if (o) {
                        version = o.ShockwaveVersion("").split("r");
                        version = parseFloat(version[0]);
                        k += "Shockwave(" + version + "),"
                    }
                    o = null;
                    try {
                        o = new ActiveXObject("WMPlayer.OCX")
                    } catch (n) {}
                    if (o) {
...
FlashExternalInterface.lastVisitedRoomId = null;
...
var loadedGameUrl;
var FlashExternalGameInterface = (function() {
    return {
        showGame: function(b) {
            var c = (b.indexOf("?") == -1) ? "?" : "&";
            loadedGameUrl = b + c + "habbo_origin=" + document.URL;
            var a = $("game-content");
            if (a.attachEvent) {
                a.attachEvent("onload", this.onGameLoaded)
            } else {
                a.onload = this.onGameLoaded
            }
            a.src = loadedGameUrl
        },
        hideGame: function() {
            $("flash-wrapper").removeClassName("hidden");
            var a = $("game-content");
            a.removeClassName("visible");
            if (a.attachEvent) {
                a.detachEvent("onload", this.onGameLoaded)
            } else {
                a.onload = null
            }
            a.src = "about:blank";
        ...
        sendGameEvent: function(a) {
            $("game-content").contentWindow.postMessage(a, loadedGameUrl)
        },
        onGameLoaded: function(a) {
            $("flash-wrapper").addClassName("hidden");
            $("game-content").addClassName("visible");
            $("flash-container").HabboGameJsAPIEventToHabbo("0.1;HabboGameAPI_game_loaded")
        }
    }
})();
var Embed = {
    embedWindowName: "embed",
    docWindowName: "habboMain",
    docWindowParams: "toolbar=yes,location=yes,directories=yes,status=yes,scrollbars=yes,resizable=yes",
    rpxWindowName: "rpxLogin",
    rpxWindowParams: "toolbar=no,location=yes,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,width=750,height=440",
    setEmbedWindowName: function() {
    ...
    openFullscreenHabbo: function(c, b) {
        var a = FlashExternalInterface.lastVisitedRoomId;
        if (a != null && a != "" && c != null) {
            c += "&roomId=" + a + "&forwardId=2"
        }
        window.name = "old-client";
        HabboClient.openOrFocus(c);
        window.location.href = b || "/embed"
    },
    decorateNaviLinks: function(a) {
        $$("#navi a").each(function(c) {
            var b = false;
            if (a && c.href.indexOf("/credits") != -1) {
                b = true;
            ...
            Event.observe(c, "click", function(g) {
                Event.stop(g);
                var d = Event.element(g);
                if (b) {
                    HabbletLoader.load("credits");
                    return
                }
...
document.observe("dom:loaded", function() {
    if ($$(".settings-container #back-link").length > 0) {
        $("back-link").insert({
            top: new Element("img", {
                src: "/v2/images/hotel.png",
                id: "back-to-main"
            })
        });
        $("back-to-main").observe("click", function() {
...
Ajax.JSONRequest = Class.create(Ajax.Base, (function() {
    ...
    return {
        ...
        request: function() {
            ...
            var g = this.options.callbackParamName,
                f = "_prototypeJSONPCallback_" + (b++),
                c = function() {
                    if (Object.isFunction(this.options.onComplete)) {
                        this.options.onComplete.call(this, d)
                    }
                    Ajax.Responders.dispatch("onComplete", this, d)
                }.bind(this);
            if (this.options.parameters[g] !== undefined) {
                f = this.options.parameters[g]
            } else {
                this.options.parameters[g] = f
            }
            this.options.parameters[g] = f;
            var e = this.options.url + ((this.options.url.include("?") ? "&" : "?") + Object.toQueryString(this.options.parameters));
            window[f] = function(h) {
                this._cleanup();
                window[f] = undefined;
                d.status = 200;
                ...
                Ajax.Responders.dispatch("onSuccess", this, d);
                c()
            }.bind(this);
...
function constructSponsorPayFrame() {
    _sp_video = new SPONSORPAY.Video.Iframe({
        ...
        greyout: false,
        callback_on_start: function(a) {
            FlashExternalInterface.clientElement.sponsorPayOnStart(a.icon_small)
        },
        callback_no_offers: function() {
            FlashExternalInterface.clientElement.sponsorPayNoOffers()
        },
        callback_on_close: function() {
            FlashExternalInterface.clientElement.sponsorPayOnClose()
        },
        callback_on_conversion: function() {
            FlashExternalInterface.clientElement.sponsorPayOnConversion()
        }
    });
...
var SponsorPay = {
    loadIntegration: function(a) {
        ...
        b.setAttribute("src", "//be.sponsorpay.com/assets/web_client.js");
